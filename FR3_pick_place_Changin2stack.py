...
class FR3StackingController(manipulators_controllers.StackingController):
    ...
    def reset(self, picking_order_cube_names: list[str] = None):
...
class FR3StackTask(Stacking):
    def __init__(
        ...
        stack_target_position: np.ndarray = None,
        ...
        super().__init__(
            ...
            stack_target_position=stack_target_position,  # ✅ 명확하게 설정
    ...
    def get_task_objects(self) -> dict:
        ...
        task_objects["stack_target"] = {"position": self.stack_target_position}  # ✅ 추가
        return task_objects
...
cube_positions = np.array([
    [-0.3, 0.6, 0.0515 / 2.0],  # 첫 번째 큐브
    [-0.3, 0.6, 0.0515 * 1.5],  # 두 번째 큐브
    [-0.3, 0.6, 0.0515 * 2.5],  # 세 번째 큐브
])

target_position = np.array([-0.3, 0.6, 0])
target_position[2] = 0.0515 / 2.0

# ✅ FR3StackTask 생성할 때 cube_initial_positions 필수
...
my_controller = FR3StackingController(
    ...
    picking_order_cube_names=["cube_1", "cube_2", "cube_3"],  # 큐브 순서 지정
...
while simulation_app.is_running():
    ...
    if my_world.is_playing():
        ...
        # ✅ 'stack_target'이 없으면 기본값 설정
        if "stack_target" not in observations:
            print("⚠ Warning: 'stack_target' not found in observations. Using default position.")
            observations["stack_target"] = {"position": target_position}  
