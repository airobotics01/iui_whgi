...
class FR3Stacking(FR3PickPlaceTask):
    def __init__(
        ...
        super().__init__(
            name=name,
            cube_initial_position=cube_initial_positions[0],  # 첫 번째 블록 pick
            cube_initial_orientation=cube_initial_orientations[0],
            target_position=target_positions[0],  # 첫 번째 블록 place
            offset=offset,
        )
        self.cube_initial_positions = cube_initial_positions
        self.cube_initial_orientations = cube_initial_orientations
        self.target_positions = target_positions
        self.current_index = 0  # 현재 스태킹 단계 인덱스
        return


    def update_target(self):
        """다음 블록을 pick & place 할 목표 위치 업데이트"""
        self.current_index += 1
        if self.current_index < len(self.target_positions):
            self.set_pick_and_place_target(
                self.cube_initial_positions[self.current_index],
                self.cube_initial_orientations[self.current_index],
                self.target_positions[self.current_index],
            )
            print(f"Next stacking step: {self.current_index}")
        else:
            print("Stacking completed!")

    def set_pick_and_place_target(self, pick_position, pick_orientation, place_position):
        """새로운 픽업 및 배치 타겟 설정"""
        self._cube_position = pick_position
        self._cube_orientation = pick_orientation
        self._target_position = place_position
